...
//		racksChosen = new HashSet<String>();
	...
	public Demand getRandomDemandToServe() {
		...
		return demand;
//		Demand demand = demandsToServe.get(new Random().nextInt(demandsToServe.size()));
//		while (true) {
...
//			rack.resetPaths(demand.getName());
	...
	public boolean serveDemandInRack(Demand d, Rack rack, Set<String> checkedRacks) {
		if (d == null)
			return false;
		sb.append("serveDemandInRack(" + d.getName() + ", " + rack.getName() + ", " + checkedRacks.size() + ") x " + ++sDC + "\n");
		...
		if (checkDD) {
			...
			if (!checkCD) {
				...
				checkCD = serveDemandInSecondRack(demand, rack, new HashSet<String>());
			}
			if (checkCD) {
...
//	public boolean serveDemandInSecondRack(Demand demand, Rack rack, Set<String> checkedRacks) {
//		sb.append("serveDemandInSecondRack(" + demand.getName() + ", " + rack.getName() + ", " + checkedRacks.size() + ") x " + ++sDCS + " #\n");
//		Demand.DataDemand dd = demand.getDataDemand();
//		Demand.CompDemand cd = demand.getCompDemand();
//		//rack.updatePathsCapacity(links);
////		Rack.Path path = rack.chooseRandomPath(dd.getDemandName(), dd.getStorage());
//		List<Rack.Path> paths = rack.chooseRandomPath(dd.getDemandName(), dd.getStorage());
//		if (path == null) {
//			sb.append("no paths #\n");
////			clearDemand(demand);
//			return false;
...
//		boolean checkCD = racks.get(racks.indexOf(secondRack)).serveCompDemand(cd);
//		if (checkCD) {
//			sb.append(demand.getName() + " COMP Served #\n");
//			return true;
//		}
...
//		Rack.Path path = rack.chooseRandomPath(dd.getDemandName(), dd.getStorage());
		...
		while (true) {
			secondRack = getRandomRack();
			String secondRackName = secondRack.getName();
			if (!secondRackName.equals(rack.getName())) {
				checkedRacks.add(secondRackName);
				break;
			}
		}
		boolean checkCD = rack.transportDemandToRack(demandName, dd.getStorage(), secondRack.getName());
		if (!checkCD) {
			sb.append("no paths #\n");
...
//		updatePaths(chosenPaths, demandName);
		checkCD = racks.get(racks.indexOf(secondRack)).serveCompDemand(cd);
		if (checkCD) {
			sb.append(demand.getName() + " COMP Served #\n");
			return true;
		}
